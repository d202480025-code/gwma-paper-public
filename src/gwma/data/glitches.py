...
def generate_glitch(
    ...
    """Generate one explicitly parameterized analytic transient."""
    ...
    if parameters.kind == "gaussian_pulse":
        ...
        # Analytic blip proxy: a Gaussian-windowed linear chirp. It is not
        # claimed to reproduce the full Gravity Spy blip population.
        sigma = max(parameters.duration / 6.0, np.finfo(float).eps)
        phase = (
